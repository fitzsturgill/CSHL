...
%%
TE = makeTE_LNL_Odor_Laser(sessions);

%% counter behavioral events
...
%% 
basepath = 'Z:\SummaryAnalyses\LNL_Odor_Laser';
...
LNL_Laser_Conditions;

%% rasters
globalTrials = 1:length(TE.filename);
xlim = [-2 6];
reversals = find(TE.BlockChange);
sessionChange = find(TE.sessionChange);
saveName = [subjectName '_lickRasters'];
...
[~, lh] = eventRasterFromTE(TE, Odor1Trials & csPlusTrials, 'Port1In', 'trialNumbering', 'global',...
...
set(gca, 'FontSize', 14)
line(repmat((xlim + [1 0])', 1, length(reversals)), [reversals'; reversals'], 'Parent', gca, 'Color', 'g', 'LineWidth', 2); % reversal lines  
line(repmat(xlim', 1, length(sessionChange)), [sessionChange'; sessionChange'], 'Parent', gca, 'Color', 'k', 'LineWidth', 2); % session lines  
% figure;

showLaser = TE.TriggerPulsePal(Odor1Trials);
showLaser(showLaser == 0) = NaN;
showLaser(showLaser == 1) = 0;
scatter(showLaser,globalTrials(Odor1Trials), 10, [1 0.75 0], '*');

subplot(1,2,2); hold on;
[~, lh] = eventRasterFromTE(TE, Odor2Trials & csPlusTrials, 'Port1In', 'trialNumbering', 'global',...
    'zeroField', 'Cue', 'startField', 'PreCsRecording', 'endField', 'PostUsRecording');
set(lh, 'Color', 'b');
[~, lh] = eventRasterFromTE(TE, Odor2Trials & csMinusTrials, 'Port1In', 'trialNumbering', 'global',...
    'zeroField', 'Cue', 'startField', 'PreCsRecording', 'endField', 'PostUsRecording');
set(lh, 'Color', 'r');
...
line(repmat((xlim + [1 0])', 1, length(reversals)), [reversals'; reversals'], 'Parent', gca, 'Color', 'g', 'LineWidth', 2); % reversal lines  
line(repmat(xlim', 1, length(sessionChange)), [sessionChange'; sessionChange'], 'Parent', gca, 'Color', 'k', 'LineWidth', 2); % session lines  
showLaser = TE.TriggerPulsePal(Odor2Trials);
showLaser(showLaser == 0) = NaN;
showLaser(showLaser == 1) = 0;
