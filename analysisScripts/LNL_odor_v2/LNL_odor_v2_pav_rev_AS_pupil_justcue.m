...
%     set(gca, 'FontSize', 14)
    ...
    subplot(1,5,2);    
    image(TE.Wheel.data.V(all_behavior_trials, wp(1):wp(2)), 'XData', [-4 3], 'CDataMapping', 'Scaled');
%     set(gca, 'CLim', [min(TE.Wheel.data.V(:)), max(TE.Wheel.data.V(:))]); 
    set(gca, 'CLim', [mean(TE.Wheel.data.V(:)) - std(TE.Wheel.data.V(:)) * 2, mean(TE.Wheel.data.V(:)) + std(TE.Wheel.data.V(:)) * 2]); 
%     line(repmat([-4; 7], 1, length(reversals)), [reversals'; reversals'], 'Parent', gca, 'Color', 'r', 'LineWidth', 2); % reversal lines    
    title('Velocity');
    ...
    subplot(1,5,3);
    image(TE.pupil.pupDiameterNorm(all_behavior_trials, wp(1):wp(2)), 'XData', [-4 7], 'CDataMapping', 'Scaled');
    set(gca, 'CLim', [nanmean(TE.pupil.pupDiameterNorm(:)) - std(TE.pupil.pupDiameterNorm(:), 'omitnan') * 2, nanmean(TE.pupil.pupDiameterNorm(:)) + std(TE.pupil.pupDiameterNorm(:), 'omitnan') * 2]); 
    set(gca, 'XLim', [-4 3]);
%     line(repmat([-4; 7], 1, length(reversals)), [reversals'; reversals'], 'Parent', gca, 'Color', 'r', 'LineWidth', 2); % reversal lines    
    colormap('parula');  
...
%     set(gca, 'CLim', [min(TE.Wheel.data.V(:)), max(TE.Wheel.data.V(:))]); 
    ...
    glmTrials_minus = csMinusTrials & punishTrials & TE.trialOutcome == 2;
    ensureFigure('Pupil_avg', 1);
    plotPupilAverageFromTE(TE, glmTrials & ~isnan(TE.pupil_cs), 'linespec', {'k'}, 'window', [-3.9 6.9]);
%     set(gca, 
    ...
    input = TE.Photometry.data(1).ZS(glmTrials, bpX2pnt(-1, 20, -4):bpX2pnt(2, 20, -4));
    input_pup = TE.pupil.pupDiameterNorm(glmTrials, bpX2pnt(-1, 20, -4) + shiftPoints :bpX2pnt(2, 20, -4) + shiftPoints);
    input_pup = input_pup';
    input_pup = input_pup(:);
...
%% DAT Model
    ...
    input = TE.Photometry.data(2).ZS(glmTrials, bpX2pnt(-1, 20, -4):bpX2pnt(2, 20, -4));
    input_pup = TE.pupil.pupDiameterNorm(glmTrials, bpX2pnt(-1, 20, -4) + shiftPoints :bpX2pnt(2, 20, -4) + shiftPoints);
    input_pup = input_pup';
    input_pup = input_pup(:);
    include = ~isnan(input_pup);
