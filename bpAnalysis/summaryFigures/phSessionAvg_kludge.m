function [avg avgX avgSEM avgAll] = phSessionAvg_kludge(SessionData, type, outcome, ax, linespec)
% kludge for new SO_training_FS protocol 1/8/2015
...
%     avgX = linspace(dataLim(1), dataLim(2), size(SessionData, 2));
    ...
    for counter = 1:length(trials)
        ...
        if counter == 1
            nSamples = size(SessionData.NidaqData{trial, 1}, 1)/dmf;
            sessionData = NaN(length(trials), nSamples);
            avgX = linspace(0, nSamples/sample_rate, nSamples);
        end
