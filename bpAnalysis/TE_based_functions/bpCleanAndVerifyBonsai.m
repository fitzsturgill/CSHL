...
%     data
    ...
    assert(length(sessions) == 1, '*** Select only one session at a time ***');
    spath = sessions.filepath;
    sname = sessions.filename;
    ...
    for counter = 1:size(fileTypes, 1)
        ...
        tomove = fileList(find(dmDelta < 3) + 1);
        nRemaining = sum(dmDelta >= 3) + 1; % remember to add on + 1 for the first file...
        if ~isempty(tomove)
