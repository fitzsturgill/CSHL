function [avg avgX avgSEM] = phSessionAvg(SessionData, type, outcome, ax, linespec)

    % I should decimate everything right from the get go at data import???
% 
    disp('phSessionAvg is deprecated and possibly incorrect');
    disp('use demodulateSession, processAnalysis_Photometry, and phPlotSession');

    fCh = 1; % fluor channel
    sample_rate = 10000;
    dmf = 100;  % decimation factor (must be a factor of sample_rate)
...
%     endField = 'PostTrialRecording';
    ...
    avgSEM = std(sessionData, 'omitnan') ./ sqrt(sum(~isnan(sessionData), 1));
