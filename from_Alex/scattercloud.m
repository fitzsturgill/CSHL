...
% For a given colormap, convert the imagesc to color values and display
% directly.  This provides compatibility with other colormaps overlain on
% top and makes the cmap input local only.
cmap = colormap;
C2 = reshape(cmap(ceil(length(cmap) * C/max(C(:))),:),[size(C,1),size(C,2),3]);
s = image(xEdges,yEdges,C2)
