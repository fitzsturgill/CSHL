...
maxpos(3) = min(maxpos(3),1920);
maxpos(4) = maxpos(4) - 60;
...
if nargin == 0
    ...
    ncols = sqrt(gridsize);                 % figure size screen scale factor
    nrows = ceil(numfigs/ncols);         % figure size screen scale factor
...
for iy = 1:nrows
    ...
    for ix = 1:ncols
        ...
        if (pnum>numfigs)
            ...
            %figure(hands(pnum))
            if ~isequal( get(hands(pnum),'Position'), [ xpos ypos xlen ylen ])
...
uistack(hands(end),'top') % Quicker than waiting for figure() - AGV
return
