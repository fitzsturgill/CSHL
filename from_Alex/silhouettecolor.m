function [s,h, SLET, silID] = silhouettecolor(X, clust, distance, colmap, varargin)
...
%
%   [S,H,SLET,silID] = SILHOUETTE(X, CLUST) plots the silhouettes, and returns the
%   silhouette values in the N-by-1 vector S, and the figure handle in H.
%   and also returns 
%           SLET (the values of the silhouette bar-plot) and 
%           silID (cell ids by cluster)
%
...
%   $Revision: 1.2.4.9 $ $Date: 2008/12/01 08:09:42 $
...
if (nargout == 0) || (nargout > 1)
    ...
    tcks=[];
    %JH to track ID
    IDsilh=[(1:length(silh))]; %JH 
    silID={}; %JH to track ID
    clusIDsilh=[];
    for i = 1:k
        bars = [bars; -sort(-silh(idx == i)); NaN(space,1)];
        [~,sortinx]=sort(silh(idx == i)); %JH
        if ~isempty(sortinx)
        clusIDsilh=IDsilh(idx == i);
        silID{i} = clusIDsilh(sortinx); %JH
        else
        silID{i} = [];     
        end
        
        bars2=[bars2; -sort(-silh(idx == i))];
        clustsize=size(-silh(idx == i),1); %JH
        if i==1 %JH
            tcks(i) = clustsize + space*i+1;  %JH
        else
            tcks(i) = tcks(i-1)+clustsize + space;   %JH
        end
        %         tcks(i) = length(bars); %JH
    end
    tcks = [1 tcks];
%     tcks = tcks - 0.5*(diff([space tcks]) + space - 1);
    ...
    inx=zeros(cn, 1);
    csize=size(colmap, 1); 
    [col space]=maximizedcolorspace(cn, colmap);
     
    %     minx=[];
    %     for j=1:cn-1
    %         inx=(j)/6;
    %         col(j+1, :)=cm(floor(csize*inx),:);
    %         minx=[minx floor(csize*inx)];
    %     end
    %%%%
    ...
    for j=1:cn
         ...
         begin=tcks(1, j);
         xa=tcks(1, j):tcks(1, j+1);
         barsh = barh(xa,bars(xa,1), 1.0,'grouped');
         in=get(barsh, 'Children');
         set(barsh, 'LineStyle', 'none')
         set(in,  'FaceColor',col(j, :))
         hold on
         
         text( -.05, mean([tcks(1, j) tcks(1, j+1)]),num2str(tcks(1, j+1)-tcks(1, j)))
