...
case 0,
 ...
 if isempty(fig), set(gcf, 'ButtonDownFcn', btdnfcn); return, end
 zoomCommand='toggle';
...
if strcmp(zoomCommand,'off'),
   ...
   set(gcf, 'ButtonDownFcn', btdnfcn); return
end % if
...
if ~isempty(ax) & any(get(ax,'view')~=[0 90]) ...
   ...
   % set(gcf, 'ButtonDownFcn', btdnfcn); return % Do nothing
else
   fZoom3d = 0;
end
...
switch zoomCommand
 case 'down',
   ...
   if ZOOM_found==0, set(gcf, 'ButtonDownFcn', btdnfcn); return, end
...
case 'getlimits', % Get axis limits
   ...
   if size(limits,2)==4 & size(limits,1)<=2, 
      if all(limits(1,[1 3])<limits(1,[2 4])), 
         getlimits = 0; out = limits(1,:); set(gcf, 'ButtonDownFcn', btdnfcn); return   % Quick set(gcf, 'ButtonDownFcn', btdnfcn); return
      else
...
if ~rbbox_mode,
   ...
   % Check for log axes and set(gcf, 'ButtonDownFcn', btdnfcn); return to linear values.
   if strcmp(get(ax,'XScale'),'log'),
      a(1:2) = 10.^a(1:2);
   end
   if strcmp(get(ax,'YScale'),'log'),
...
if zoomx,
   if a(1)==a(2), set(gcf, 'ButtonDownFcn', btdnfcn); return, end % Short circuit if zoom is moot.
   set(ax,'xlim',a(1:2))
   h = list(1);
   while h ~= ax,
      set(h,'xlim',a(1:2))
...
if zoomy,
   if a(3)==a(4), set(gcf, 'ButtonDownFcn', btdnfcn); return, end % Short circuit if zoom is moot.
   set(ax,'ylim',a(3:4))
   h = list(2);
   while h ~= ax,
      set(h,'ylim',a(3:4))
...
%GET_CURRENTPOINT set(gcf, 'ButtonDownFcn', btdnfcn); return equivalent linear scale current point
...
%GET_XLIM set(gcf, 'ButtonDownFcn', btdnfcn); return equivalent linear scale xlim
xlim = get(ax,'xlim');
if strcmp(get(ax,'XScale'),'log'),
   xlim = log10(xlim);
end
...
%GET_YLIM set(gcf, 'ButtonDownFcn', btdnfcn); return equivalent linear scale ylim
ylim = get(ax,'ylim');
if strcmp(get(ax,'YScale'),'log'),
   ylim = log10(ylim);
end
