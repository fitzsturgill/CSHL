...
%	set(h, 'Enable', 'off');	
    ...
    try
        global state
        disp(['Laser 2 power = ' ...
               num2str(state.pcell.pcellPowerCal2(1+round(state.pcell.pcellOffset2)))...
                ' mW']);
        state.pcell.pcellOffsetPower2=state.pcell.pcellPowerCal2(1+round(state.pcell.pcellOffset2));
    catch
        state.pcell.pcellOffsetPower2=-1;
    end
    updateHeaderString('state.pcell.pcellOffsetPower2');
    try
        global state
        disp(['Laser 1 power = ' ...
               num2str(state.pcell.pcellPowerCal1(1+round(state.pcell.pcellOffset1)))...
                ' mW']);
        state.pcell.pcellOffsetPower1=state.pcell.pcellPowerCal1(1+round(state.pcell.pcellOffset1));
    catch
        state.pcell.pcellOffsetPower1=-1;
    end
    updateHeaderString('state.pcell.pcellOffsetPower1');    
	handleChange;
	set(h, 'Enable', 'on');	
...
function varargout = selectXYZButton_Callback(h, eventdata, handles, varargin)
	...
	if state.internal.selectionChannel<=3
		figure(state.internal.MaxFigure(state.internal.selectionChannel));
	elseif state.internal.selectionChannel==4
		figure(state.internal.refFigure);
	end
	[y, x]=getBoxCoords([], 1);	% get a box with the coords sorted by X
