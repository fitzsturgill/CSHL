function makeNewPcellRepeatedOutput
	...
	global state
	actualOutputRate = get(state.daq.pcellGrabOutput, 'SampleRate');
	...
	shutterLead=round(state.internal.lengthOfXData*state.pcell.shutterDelay/(state.acq.msPerLine*1000));
		...
		for lineCounter=1:size(state.blaster.allConfigs{state.blaster.currentConfig, 2},1)
			...
			if pulseNumber
				if state.blaster.widthFromPattern
					width=state.phys.pulses.pulseWidthList(pulseNumber);
				else
				...
				if state.blaster.powerFromPattern
					power1=0;
					power2=state.phys.pulses.amplitudeList(pulseNumber);	% this is a cludge and this should be set so that each channel is controlled separately
				else
				...
				try
					for patternCounter=1:max(state.phys.pulses.patternRepeatsList(pulseNumber), 1)
						start=state.phys.pulses.delayList(pulseNumber) + (patternCounter-1)*state.phys.pulses.patternISIList(pulseNumber);
						for counter=1:state.phys.pulses.numPulsesList(pulseNumber)
							...
							start=start + state.phys.pulses.isiList(pulseNumber);
						end	
					end
				catch
