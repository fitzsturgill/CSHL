function updateReferenceImage
	...
	try
		...
		im=mean(state.acq.acquiredData{state.acq.trackerChannel},3);
...
% 						'Parent', state.internal.refAxis, 'LineWidth', 2.5);
	...
	if state.analysis.analysisMode==2 | (state.analysis.analysisMode==4 & state.acq.lineScanCB)
		if state.acq.lineScanCB & (size(state.analysis.roiDefs,1) >= state.analysis.numberOfROI)
	...
	elseif state.analysis.analysisMode==3 | (state.analysis.analysisMode==4 & ~state.acq.lineScanCB)
		if ~state.acq.lineScanCB & (size(state.analysis.roiDefs2D,1) >= state.analysis.numberOfROI)
			colorList='brkgmcykkkkkkkkkk';
			for roiCounter=1:state.analysis.numberOfROI
