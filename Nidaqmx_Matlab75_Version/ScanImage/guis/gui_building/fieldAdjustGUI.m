...
% Last Modified by GUIDE v2.5 08-Apr-2005 13:19:50
...
%	set(hObject, 'Enable', 'off');
	try
		...
		index = round(mean(y)-1) * state.internal.lengthOfXData + round(state.internal.lengthOfXData*(state.internal.fractionStart+mean(x)*state.internal.fractionPerPixel));
		...
		% calculate angle
        if (x(2) ~= x(1))
		    state.acq.scanRotation = state.acq.scanRotation - atan((y(2)-y(1))/(x(2)-x(1)))*180/pi;
        end
...
%	set(hObject, 'Enable', 'off');
	try
		...
		if isempty(findobj(gcf, 'Type', 'axes'))
       	...
       	0.001*(state.acq.lineDelay)/state.acq.msPerLine*state.internal.samplesPerLine;
...
% --- Executes on slider movement.
function piezoZslider_Callback(hObject, eventdata, handles)
