function applyBlasterParking(config)
            ...
            if pulseNumber
                ...
                if state.blaster.indexList(pos)
                    if state.blaster.screenCoord
                        ...
                        X=state.acq.rotatedMirrorData(state.blaster.indexList(pos), 1);
                        Y=state.acq.rotatedMirrorData(state.blaster.indexList(pos), 2);
                ...
                % 					power=state.phys.pulses.amplitudeList(pulseNumber);
                % 				else
    ...
    if ~isempty(parkParams)
        acqLength=state.acq.msPerLine*state.acq.linesPerFrame*state.acq.numberOfFrames*1000; 	% in ms
