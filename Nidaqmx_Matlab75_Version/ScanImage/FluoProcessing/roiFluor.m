function out=roiFluor(imData, roiDefs, offset)
	...
	global state
	if state.analysis.analysisMode==2 | (state.analysis.analysisMode==4 & state.acq.lineScanCB)	% line scan analysis
	...
	elseif state.analysis.analysisMode==3 | (state.analysis.analysisMode==4 & ~state.acq.lineScanCB) % 2d analysis
