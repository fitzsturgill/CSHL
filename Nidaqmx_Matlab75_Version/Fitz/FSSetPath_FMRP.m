function FSSetPath_FMRP(incNum)
% function FSSetPath_FMRP(incnum)
    global state
    ...
    basedir='N:\Microscope\Fitz\FMRP\';
    cd(basedir);
    ...
    while(notFoundDir)
       dirname=[basedir 'fmrp_' num2str(incNum)];
	   if incNum ==1;
		   break
	   end
       try
           ...
           notFoundDir=0;
           if bool
               disp(['***Experiment Number too high. --- fmrp_' num2str(incNum) ' does not exist on Microscope.--- Set to lower value.****']);
               return
           end
       end 
    end
    ...
    state.files.baseName=['fmrp_' num2str(incNum) '_'];
