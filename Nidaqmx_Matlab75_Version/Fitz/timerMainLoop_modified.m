function timerMainLoop_modified
	global state gh fsBoxWaveStruct fsBoxWave
	...
	if state.internal.firstTimeThroughLoop==0 | state.acq.externalTrigger
		...
		%modified
		if exist('fsBoxWaveStruct')
			disp('hello');
			fsMean=mean2(state.acq.acquiredData{fsBoxWaveStruct.channel}(fsBoxWaveStruct.x0:fsBoxWaveStruct.x1,fsBoxWaveStruct.y0:fsBoxWaveStruct.y1));
			fsOffset=0;
			eval(['fsOffset=state.acq.binFactor*state.acq.pmtOffsetChannel' num2str(fsBoxWaveStruct.channel) ';']);
			subtracted=fsMean-fsOffset;
			fsBoxWave.data=[fsBoxWave.data subtracted];
		else
			disp('goodbye');
		end
