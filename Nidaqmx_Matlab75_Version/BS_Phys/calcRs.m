function [rin, rs, cm, error]=calcRs(data, dx, pulseAmp, pulseStart, pulseLength, baselineStart, baselineEnd)
		...
		if pulseAmp>0
			[peak, peakloc]=max(data(round(pulseStart/dx)+1:1+round((pulseStart+pulseLength)/dx)));;
		else
			[peak, peakloc]=min(data(round(pulseStart/dx)+1:1+round((pulseStart+pulseLength)/dx)));;
		end
		...
		if pulseAmp>0
			above=find(data(round(pulseStart/dx)+1:round((pulseStart+pulseLength)/dx))>(peak-endline)/5+endline);
			if isempty(above)
				error=1;
				return;
			end
