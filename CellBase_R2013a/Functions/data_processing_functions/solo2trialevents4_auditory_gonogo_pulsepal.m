function TE = solo2trialevents4_auditory_gonogo_pulsepal(filepath,ifsave)
%SOLO2TRIALEVENTS4_AUDITORY_GONOGO   Create trial events structure.
%   SOLO2TRIALEVENTS4_AUDITORY_GONOGO(SESSPATH) creates trial events 
...
%
%   SOLO2TRIALEVENTS4_AUDITORY_GONOGO(SESSPATH,IFSAVE) uses an addidional
%   logical input argument do determine whether to save the results.
%
%   See also MAKETRIALEVENTS2_AGONOGO and DEFINEEVENTSEPOCHS_AGONOGO.
...
TE.LightStimulation3 = nan(1,ntrials);
...
TE.NoPokeITIBegins = cell(1,ntrials);
TE.NoPokeITIEnds = cell(1,ntrials);
TE.TotalNoPokeITI = nan(1,ntrials);
...
% Correct side
lefttrials = find(saved.auditory_gonogo_pulsepal_TrialTypeList(1:ntrials)=='l');
righttrials = find(saved.auditory_gonogo_pulsepal_TrialTypeList(1:ntrials)=='r');
...
TE.StimulusID = saved.auditory_gonogo_pulsepal_TrialTypes(1:ntrials);
TE.StimulusID = [TE.StimulusID(2:end) NaN];  % remove first trial

% Stimulus duration / sound intensity on every trial
TE.StimulusDuration = saved.auditory_gonogo_pulsepal_StimDurList(1:ntrials);
TE.StimulusDuration = [TE.StimulusDuration(2:end) NaN];  % remove first trial
TE.SoundIntensity = saved.auditory_gonogo_pulsepal_StimDurList(1:ntrials);   % it is actually sound intensity; however, I keep the redundant StimulusDuration field to avoid errors in other programs - BH
...
TE.LightStimulation = saved.auditory_gonogo_pulsepal_LightStimList(1:ntrials);
TE.LightStimulation = [TE.LightStimulation(2:end) NaN];  % remove first trial
TE.LightStimulation2 = saved.auditory_gonogo_pulsepal_LightStimulation2(1:ntrials);
TE.LightStimulation2 = [TE.LightStimulation2(2:end) NaN];  % remove first trial
TE.LightStimulation3 = saved.auditory_gonogo_pulsepal_PulsePalStimulationTrials(1:ntrials)';
TE.LightStimulation3 = [TE.LightStimulation3(2:end) NaN];  % remove first trial

% Define variables trialwise
countr = 0;
for iT = 2:ntrials
    countr = countr + 1;
    TE.StimulusOn(countr) = saved_history.ProtocolsSection_parsed_events{iT}.states.deliverstim(1);
    TE.StimulusOff(countr) = saved_history.ProtocolsSection_parsed_events{iT}.states.deliverstim(2);
    TE.GoSignalOn(countr) = saved_history.ProtocolsSection_parsed_events{iT}.states.deliverstim(1);
    TE.GoSignalOff(countr) = saved_history.ProtocolsSection_parsed_events{iT}.states.deliverstim(2);
%     TE.ReactionTime(countr) = diff(saved_history.ProtocolsSection_parsed_events{iT}.states.beginresponse);
    ...
    % Stimulus
    TE.SignalModality{countr} = saved.auditory_gonogo_pulsepal_SignalModality;
    ...
    TE.LeftWaterValveDur(countr) = saved_history.WaterValvesSection_LeftWValveTime{iT};

    try TE.RightWaterValveOn(countr) = saved_history.ProtocolsSection_parsed_events{iT}.states.rdeliver(1); catch; end
    TE.RightWaterVolume(countr) = saved_history.WaterValvesSection_Right_volume{iT};
    TE.RightWaterValveDur(countr) = saved_history.WaterValvesSection_RightWValveTime{iT};
...
TE.DeliverFeedback(TE.Hit==1) = TE.LeftWaterValveOn(TE.Hit==1);
TE.DeliverFeedback(TE.FalseAlarm==1) = TE.RightPunishBegin(TE.FalseAlarm==1);
TE.PseudoStimulusOn = TE.DeliverFeedback - TE.TotalReactionTime;  % to allow sorting based on RT

% Assign a Block Number
% we are going to assume there was only 1 Block size ... .i.e. only one value for
% TrialsPerBlock per session.
TE.TrialsPerBlock(:) = saved_history.auditory_gonogo_pulsepal_TrialsPerBlock{1};
TE.TrialsPerBlock = [TE.TrialsPerBlock(2:end) NaN];  % remove first trial

blocklength = saved_history.auditory_gonogo_pulsepal_TrialsPerBlock{1}*length(unique(TE.StimulusDuration))*2;
...
TE.PseudoStimulusOn = TE.PseudoStimulusOn - TE.TrialStart;
for k = 1:ntrials
    ...
    TE.NoPokeITIBegins{k} = TE.NoPokeITIBegins{k} - TE.TrialStart(k);
    TE.NoPokeITIEnds{k} = TE.NoPokeITIEnds{k} - TE.TrialStart(k);
end
