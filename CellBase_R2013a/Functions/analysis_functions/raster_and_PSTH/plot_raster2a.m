function fhandle0 = plot_raster2a(stimes,time,valid_trials,COMPTRIALS,TAGS,EventTimes,window_margin,ev_windows,sort_var,params,varargin)
%PLOT_RASTER2A   Plot spike raster.
...
%       STIMES: spike times
%       VALID_TRIALS: trials to include in the plot
...
% default_args.EventMarkerWidth = 0.02;
default_args.EventMarkerHeight = 2;
default_args.SpikeWidth = 1.5; % 0.5
    ...
    if NumTrials(iCOMP) > 0
        ...
        for iTRIAL = 1:NumTrials(iCOMP)
            ...
            if ~iscell(ev_windows)
                ...
                ok_spikes = spikes(spikes>win(1)&spikes<=win(2));
                ...
                win = [max(ev_windows{indTRIAL}(1,:)',time(1)) min(ev_windows{indTRIAL}(2,:)',time(end))]; 
                NUMlicks = length(spikes);
                ok_spikes = cell(NUMlicks,1);
                for k = 1:NUMlicks
                    cntr = cntr + 1;
                    ok_spikes{k} = spikes{k}(spikes{k}>win(k,1)&spikes{k}<=win(k,2));
                    if ~isempty(ok_spikes{k})
                        if size(ok_spikes{k},1) == 1
                            lnx = [lnx [ok_spikes{k}; ok_spikes{k}]];
                            lny = [lny [(nt(cntr)-0.5)*ones(1,length(ok_spikes{k})); (nt(cntr)+0.5)*ones(1,length(ok_spikes{k}))]];
                        else
                            lnx = [lnx [ok_spikes{k} ok_spikes{k}]'];
                            lny = [lny [(nt(cntr)-0.5)*ones(1,length(ok_spikes{k})); (nt(cntr)+0.5)*ones(1,length(ok_spikes{k}))]];
                        end
        ...
        if iscell(ev_windows)    % deals with lick-aligned raster - BH
            L = line(lnx,lny,'Color','k','Linewidth',par.SpikeWidth);   % plots spikes as a single line object
        end
