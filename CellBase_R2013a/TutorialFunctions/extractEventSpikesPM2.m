function [event_stimes,event_windows] = extractEventSpikesPM2(cellid,events,TE)
...
%   Edit log: AK 11/06, ZFM 10/04, BH 6/27/11 PM 7/03/14
...
for iE = 1:NUMevents
    ...
    if isfield(TE,TriggerEvent1) && isfield(TE,TriggerEvent2)
        NUMtrials = TE.nTrials;
        if ~iscell(TE.(TriggerEvent1))    % original code
            EventTimes1 = TE.(TriggerEvent1) + TE.TrialStartAligned  + offset1;
            EventTimes2 = TE.(TriggerEvent2) + TE.TrialStartAligned + offset2;
            ...
            for iT = 1:NUMtrials
                ...
                EventTimes1{iT} = EventTimes1{iT} + TE.TrialStartAligned(iT) + offset1;
                EventTimes2{iT} = EventTimes2{iT} + TE.TrialStartAligned(iT) +  offset2;
