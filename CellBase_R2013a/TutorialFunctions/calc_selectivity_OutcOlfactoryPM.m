function [out1 out2] = calc_selectivity_OutcOlfactoryPM(cellid,EpochName,nboot,eventtype)
% 
...
% try
    ValidTrials = selecttrial(TE,'CompletedTrial==1 & Modality==1');
    CorrectOlfTrials        = intersect(find(TE.CorrectChoice==1),ValidTrials);
    WrongOlfTrials       = intersect(find(TE.CorrectChoice==0),ValidTrials);
% catch
...
[Doutmod, Poutmod] = rocarea(RATE(CorrectOlfTrials),RATE(WrongOlfTrials),'boot',nboot,'scale');
